 ...
 *
 * Fix under test: the household push coordinator's capped exponential
 * backoff (`scheduleRetry` with phase 'offline-backoff') keeps retrying
 * until a push succeeds, cleared on success.
 */
...
test('push failure schedules a retry; success clears it', () => {
  ...
  const installIndex = src.indexOf('applyRemotePatch');
