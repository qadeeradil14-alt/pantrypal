-- Allow any household member (not only the active shopper) to cancel/reset
-- the active trip via the household snapshot, so a member is never locked
-- behind a session they didn't start.
--
-- private.can_update_household_snapshot_as_member's existing early-return
-- ("v_old_session is not null and shopperId is empty/mine -> true") only
-- covers the shopper (or a shopperless session). Any other member's write
-- falls through to the narrow entries/removedItemIds/removedAt/storeQueue
-- diff checks below it, which assume a small in-place item edit — a full
-- activeSession -> null transition (client's END_TRIP) fails those checks
-- and is rejected, even though the client-side permission fix now allows
-- any member to dispatch END_TRIP locally. Without this, a member's cancel
-- would apply on their own device but silently fail to sync to the rest of
-- the household.
--
-- This adds exactly one narrow carve-out: v_old_session is not null (a trip
-- is running) and v_new_session is null (the write clears it) is always
-- allowed, for any household member. It is deliberately scoped to that one
-- transition — clearing the session — and changes nothing else: a member
-- attempting any other shape of change to a trip they're not shopping still
-- falls through to the unchanged, stricter checks below.
...
begin
  ...
  -- Cancel/reset: any member may collapse a running session to null. Narrow
  -- by construction — only this exact transition is allowed, nothing else.
  if v_old_session is not null and v_new_session is null then
    return true;
  end if;
